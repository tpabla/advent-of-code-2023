fn main() {
    ...
    println!("Advent of code, Day 9, Part 2");
...
fn get_next_value(values: Vec<i64>) -> i64 {
    ...
    values.first().expect("an ending number") + end_values.iter().sum::<i64>()
}
...
mod tests {
    ...
    fn it_works_for_first_test_case() {
        assert_eq!(get_next_value(vec![0,3,6,9,12,15]),-3)
    }

    #[test]
    fn it_works_for_second_test_case() {
        assert_eq!(get_next_value(vec![1,3,6,10,15,21]),0)
    }

    #[test]
    fn it_works_for_third_test_case() {
        assert_eq!(get_next_value(vec![10,13,16,21,30,45]),5)
    }
...
10 13 16 21 30 45".to_string();
        assert_eq!(process_input(input_string), 2);
